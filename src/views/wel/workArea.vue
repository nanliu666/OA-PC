		...
		<el-col :span="24">
			<div class="workArea-wrap">
				<el-row :gutter="20">
					<el-col :span="12">
						<!-- 代办事项 -->
						<div class="todolist-wrap" v-loading="todoLoading">
							<tagCom
							...
							>
								<!-- 待处理 -->
								<div v-if="toDoListData.length" slot="Pending">
									<div v-for="item in toDoListData" :key="item.id" class="item-row">
										<div class="text-box">
											<p @click="jumpToDetail(item.type, item.bizId)">
												【{{ item.type | filterType }}】{{ item.title }}
											</p>
											<span v-if="ifShowWarn(item.createTime)"
								...
								<!-- 预警 -->
								<div v-if="warningList.length" slot="Warning">
									<div v-for="item in warningList" :key="item.id" class="item-row">
										<div class="text-box">
											<p @click="jumpToDetail(item.type, item.bizId)">
												【{{ item.type | filterType }}】{{ item.title }}
											</p>
								...
								<div
									...
									class="view-all"
								>
									<el-button type="text" size="medium" @click="goTodoCenter">
										查看全部
									</el-button>
								</div>
							</tagCom>
					...
					<el-col :span="12">
						...
						<div class="news-wrap">
							...
							>
								<div v-if="msgWorkList.length" slot="workNews">
									<el-tooltip
										:enterable="false"
								...
								<div v-else slot="workNews">
									<div class="no-msg-box">暂时没有工作消息通知</div>
								</div>
								<div v-if="msgSystemList.length" slot="systemNews">
									<el-tooltip
										:enterable="false"
								...
								>
									<el-button type="text" size="medium" @click="goMsgCenter">
										查看全部
									</el-button>
								</div>
							</tagCom>
